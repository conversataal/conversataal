...
Elm.Users.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var dave = {name: "Dave",location: "Scotland",age: 33};
   var noah = {name: "Noah",location: "N/A",age: 23};
   var users = _U.list([noah,dave]);
   var User = F3(function (a,b,c) {    return {name: a,location: b,age: c};});
   return _elm.Users.values = {_op: _op,User: User,noah: noah,dave: dave,users: users};
};
Elm.Blog = Elm.Blog || {};
Elm.Blog.Index = Elm.Blog.Index || {};
Elm.Blog.Index.make = function (_elm) {
   "use strict";
   _elm.Blog = _elm.Blog || {};
   _elm.Blog.Index = _elm.Blog.Index || {};
   if (_elm.Blog.Index.values) return _elm.Blog.Index.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var _op = {};
   var viewBloggerInfo = function (user) {
      var words = A2($String.join,"",_U.list([user.name," is currently in ",user.location," and is ",$Basics.toString(user.age)," years old!"]));
      return A2($Html.div,_U.list([]),_U.list([$Html.text(words)]));
   };
   var view = A2($Html.div,_U.list([]),A2($List.map,viewBloggerInfo,$Users.users));
   return _elm.Blog.Index.values = {_op: _op,viewBloggerInfo: viewBloggerInfo,view: view};
};
Elm.Index = Elm.Index || {};
...
Elm.Renderer.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Blog$Index = Elm.Blog.Index.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Index = Elm.Index.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$Renderer = Elm.Native.Renderer.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var render = $Native$Renderer.toHtml;
   var index = Elm.Native.Port.make(_elm).outbound("index",function (v) {    return v;},render($Index.view));
   var blogindex = Elm.Native.Port.make(_elm).outbound("blogindex",function (v) {    return v;},render($Blog$Index.view));
   return _elm.Renderer.values = {_op: _op,render: render};
};
var fs = require('fs');
